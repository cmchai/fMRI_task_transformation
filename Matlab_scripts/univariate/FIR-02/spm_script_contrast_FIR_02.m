% Script to run contrasts from the result of FIR-02
% Author: Mengqiao Chai (Mengqiao.Chai@ugent.be)
% Last update on: 22nd, Apr, 2024

% 2 regressors of interest, including RG-long-qc, TF-long-qc.
% for each regressor, 10 basis function estimations(FIR-01B), whose name looks like RG-long-qc*bf(1), RG-long-qc*bf(2)...
% contrasting, for each TR, the basis function estimation between RG and TF
...
subjs = [2,3,5,7:11,13:17,20:44,46:49];

% the root folder of the FLM result
FLM_root_dir = '/Volumes/extdrive/Task_Transform_GLM/FIR-02/results';

% define how many TRs that we want to contrast
sumTR = 10;
...
% loop over each TR and define the names of the contrast
for TR = 1:sumTR
    cnames{TR} = ['long-RG>TF*bf(', num2str(TR), ')'];
    connames_pos{TR} = [convertCharsToStrings(['RG-long-qc*bf(', num2str(TR), ')'])];
    connames_neg{TR} = [convertCharsToStrings(['TF-long-qc*bf(', num2str(TR), ')'])];
    cnames{TR+sumTR} = ['long-TF>RG*bf(', num2str(TR), ')'];
end
