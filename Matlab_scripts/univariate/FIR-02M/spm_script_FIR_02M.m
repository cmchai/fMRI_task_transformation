% Script to run FIR-02M for the Task Transformation Paradigm
% Aimed for time-evolved compositional decoding analysis
% images are preprocessed using fMRIprep with SYN as the sdc approach
% For each functional run, there are 5 dummy scans that need to excluded before GLM
% Images should NOT be smoothed before the GLM for the follow-up decoding
% should be ____ predictors per subject(if there are error trials per block)
...
% define the correct path of all the folders
event_rootdir = '/Users/mengqiao/Documents/fMRI_task_transform/MRI_data/Task_transform/first_level/multivariate/FIR-02M/events';
output_rootdir = '/Volumes/extdrive/Task_Transform_GLM/FIR-02M/results';
...
conf_regs_rootdir = '/Users/mengqiao/Documents/fMRI_task_transform/MRI_data/Task_transform/first_level/univariate/GLM-01/events';  % which DOES NOT include stimulus and q2 convolved onset regressors since, in theory, they would not confound the mulit-variate pattern of task identity
...
subjs = [41:44,46:49];                     % subjects that will be run
...
for subj = subjs
    ...
    for ses = 1:n_ses
        ...
        for run = 1:n_run
            ...
            % select the corresponding image after preprocessing WITHOUT smoothing
