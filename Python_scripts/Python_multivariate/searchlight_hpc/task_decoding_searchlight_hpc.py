...
#%% Create the numpy array to save all the searchlight results for all participants(delete this part if using working mode for parallel processing)
...
for subj_idx, subj in enumerate(subjs): # loop over subjects
    ...
    if os.path.isfile(GM_crop_mask_path): # if the cropped mask file already exist, then load that file 
        ...
        nib.nifti1.save(GM_mask_crop, subj_dir)
