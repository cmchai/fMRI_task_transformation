...
root_o_dir <- "/Users/mengqiao/Documents/fMRI_task_transform/MRI_data/Task_transform/first_level/univariate/GLM-02/events"

all_subjs <- c(2:5,7:11,13:17,20:44,46:49)
subjs <- c(7:11,13:17,20:44,46:49) # subjects to run
...
for (subj in subjs) { # loop over each participant
  ...
  # Here we picked only accurate long trials
  trials_int <- bigdf %>%
    filter(accuracy == TRUE & CTI_length == "long")
  ...
  ### Events of No interest 2: short trials ----
  trials_short <- bigdf %>%
  ...
  rm(bigdf, trials_int, trials_error, trials_short)
}
