...
for (subj in subjs) { # loop over each participant
  ...
  ##################  Generate and Save Nuisance regressors(*.txt) files ##############-----
  ...
  sum_dummyscans <- 5 # IMPORTANT! the data corresponding to the dummy scans need to be discarded. 
  ...
  for(ses in 1:sum_ses) {
    ...
    for(run in 1:sum_run) {
      ...
      conf_file_df <- read_tsv(file.path(func_dir, name_conf_file)) %>%
        ...
        slice(-(1:sum_dummyscans)) # Important! get rid of data from dummy scans
  ...
  rm(bigdf, trials_int, trials_error)
}
