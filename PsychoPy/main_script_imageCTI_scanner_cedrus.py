...
if scanning == True: 
    globalVars["leftKey"] = 1
    globalVars["rightKey"] = 2
    keyList = [1, 2,'escape']
else:
...
CTIup_obj_img = visual.ImageStim(win, image = 'images/error.jpg', size = (0.006, 0.006), pos = (0, 0.25), units = "height")
CTImid_obj_img  = visual.ImageStim(win, image = 'images/error.jpg', size = (0.006, 0.006), pos = (0, 0), units = "height")
CTIlow_obj_img  = visual.ImageStim(win, image = 'images/error.jpg', size = (0.006, 0.006), pos = (0, -0.25), units = "height")
...
if scanning == True:
    ...
    win=win,
    name='text',
    text='Waiting for scanner, please get ready',
    font='Arial',
    pos=(0, 0),
    height=0.1,
    wrapWidth=None,
    ...
    st = s.ScannerTrigger.create(
            ...
            esc_key='escape')
    st.open()
    buttonBox = st.port # define the cedrusBox

#%% detect the Cedrus button-box
# if the Cedrus buttonBox is not initialized yet
# if scanning == True:
#     try: 
...
def check_XID_response(buttonBox, maxWait, keyList, responseClock):
    responseClock.reset()
    respKey = None
    respTime = float()
    ...
    while responseClock.getTime() <= maxWait: # maxWait
        ...
        if buttonBox.has_response():
            respTime = responseClock.getTime();
            
            # Get the event of the next response in the queue
    ...
    return respKey, respTime # if the respKey is actually the trigger, then the respTime should be 2 seconds
...
for blockIdx, blockID in enumerate(blockIDs, runNo_start - 1):
    ...
    for trialIdx, trialDict in enumerate(trialIterator):
        ...
        clock_stim.reset()
        response, rt = check_XID_response(buttonBox = buttonBox, 
        ...
        if response in keyList[0:2]:# if button box is pressed
        ...
        else: # button box was not pressed
            resp_onset = np.nan
