...
if scanning == True: # need to confirm with GIFMI
    globalVars["leftKey"] = 49
    globalVars["rightKey"] = 51
else:
...
if running == True:
    dlg = gui.DlgFromDict(dictionary = info, title = 'Experiment Setup', order = ['Subject No.'])
    if not dlg.OK: core.quit()
    if info['Subject No.'] == '': core.quit()

if debugging == True:
    subjNo = random.randrange(1, 51) # pick an random integer from 1 to 50
else:
...
# response mappings
all_respMaps = [(0,0,0), (0,0,1), (0,1,0), (1,0,0), (0,1,1), (1,1,0), (1,0,1), (1,1,1)]
responseMappings = random.choice(all_respMaps)
...
instr_respMap = visual.ImageStim(win, image = respMap_inst, pos = (0, -0.2), units = "height")
...
for blockIdx, blockID in enumerate(blockIDs):
    ...
    if scanning == True:
        ...
        try:
            ...
            triggered = st.waitForTrigger(skip=5) # It's True when it receives a trigger, skip 5 volumns
            print('----------Trigger OK')
            #logging.flush()
    ...
    ####### leave 16s to include the whole cycle of the HRF response for the last event #######
    block_end.draw(); win.flip();
    core.wait(16)
