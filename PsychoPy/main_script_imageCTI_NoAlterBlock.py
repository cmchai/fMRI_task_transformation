...
if running == True: # if running the experiment
    ...
    cwd = 'D:\\Ghent_Braem\\Experiment\\1st_fMRI\\Behaviour_Pilot\\2nd_InLab\\Mengqiao\\fMRI_behav_pilot\\experiment_psychopy'
...
if debugging == True:
    subjID = 'fakeID2'
    CB_no = 22
    info['reg_color'] = "#B200ED"
else:
    subjID = str(info['Participant ID'])
    CB_no = int(info['Counter balance No.'])

#%% defining all the between subject variables
fixationColors, blockIDs = genCon.bet_sub_randomization2(CB_no, info['reg_color']) # if CB_no is an even number, then start with regular block
...
ITI_obj = visual.TextStim(win, text='+', height=0.07, pos = (0, 0),  bold=False, color=(-1,-1,-1))
...
for blockIdx, blockID in enumerate(blockIDs):
    ...
    if debugging == True:
        trialIterator = trialIterator[0:5] # only run 10 trials per block for debugging purpose
    random.shuffle(globalVars["ITIs"])
    #print(blockIdx, blockID)
    
    # show block instruction and start the block
    if blockID[0:2] == "RG":
