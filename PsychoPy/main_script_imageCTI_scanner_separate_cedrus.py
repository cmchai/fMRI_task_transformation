...
#%% Collect demographic info and RUN number
# Dialogue Box
info = {'Subject No.': '0', # the subject number
        'RUN No.': '0' }    # the run number

if running == True:
    dlg = gui.DlgFromDict(dictionary = info, title = 'Experiment Setup', order = ['Subject No.', 'RUN No.'])
    if not dlg.OK: core.quit()
    if info['Subject No.'] == '': core.quit()
    subjNo = int(info['Subject No.'])
    runNo = int(info['RUN No.'])
else:
    subjNo = random.randrange(1, 10) # pick an random integer from 1 to 10
    runNo = random.randrange(1, 9) # randomly pick one number from 1 - 8
    
subjID = "subj_" + str(subjNo)
blockIdx = runNo - 1  # one block per run
...
with open(cwd +"\\iterators\\scan_cedrus\\exp_iterator_subj" + str(subjNo) + ".json", "r") as read_it:
...
# to create a function to record the XID response
...
# define the identity of the block as well as the run, and the trial iterator
runID = "run_" + str(runNo)
blockID = blockIDs[blockIdx]            # which SHOULD BE THE SAME AS IN THE blocktrialiterator
...
# the shuffled ITI for the whole block
...
# behavioural data file directory and names
datafile_path = cwd + '\\data\\'; print(datafile_path)
...
for trialIdx, trialDict in enumerate(trialIterator):
    ...
    clock_stim.reset()
    response, rt = check_XID_response(cedrusBox = cedrusBox, 
                                      maxWait = globalVars["ddlResponse"]/1000, 
                                      keyList = keyList, 
                                      responseClock = clock_stim)
    stim_offset = clock_trigger.getTime()
    
    # retrieve response, evaluate accuracy, and show feedback
    if response != 4: # button box is used
...
################################################################################################
if blockIdx < 7: # the feedback for the first 7 blocks
    blockFb = ""
...
else: # if the last block close the whole experiment 
